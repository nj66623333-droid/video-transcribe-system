...
# 优先加载便携依赖包目录
_portable_lib = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'portable_lib')
if os.path.isdir(_portable_lib):
    sys.path.insert(0, _portable_lib)
...
五、处理速度参考（约10-15分钟视频）
   硅基流动（Qwen2.5）:   ~9 分钟（转录+四步精修+摘要）
   本地 Ollama（gemma4）: ~19 分钟（比云端慢约10分钟）
   仅转录（关闭AI增强）:  ~2 分钟
